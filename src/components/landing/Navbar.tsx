      ...
      >
        <div className="mx-auto max-w-7xl px-6 lg:px-8 h-full flex items-center justify-between">
          <Link href={`/${locale}`} className="flex items-center gap-3 group">
            <span className={`flex h-10 w-10 items-center justify-center rounded-xl text-sm font-bold shadow-md transition-all duration-300 ${scrolled ? 'bg-gradient-to-br from-land-green-600 to-land-green-700 text-white' : 'bg-white/10 backdrop-blur-sm border border-white/20 text-white'}`}>A</span>
          ...
          <nav className="hidden lg:flex lg:items-center lg:gap-1">
            {links.map(link => (
              <Link key={link.href} href={link.href} className={`rounded-lg px-4 py-2.5 text-sm font-medium transition-all duration-200 ${scrolled ? 'text-gray-600 hover:text-gray-900 hover:bg-gray-50' : 'text-white/70 hover:text-white hover:bg-white/10'}`}>{link.label}</Link>
            ))}
          </nav>
          <div className="hidden lg:flex lg:items-center lg:gap-3">
            <LocaleSwitcher currentLocale={locale as Locale} />
            {!signedIn && <Link href={`/${locale}/login`} className={`rounded-lg px-4 py-2.5 text-sm font-medium transition-all duration-200 ${scrolled ? 'text-gray-600 hover:text-gray-900 hover:bg-gray-50' : 'text-white/70 hover:text-white hover:bg-white/10'}`}>{loginLabel}</Link>}
      ...
      <div className={`fixed inset-0 z-[70] bg-land-dark lg:hidden flex flex-col transition-opacity duration-300 ${mobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} role="dialog" aria-modal={mobileOpen} aria-label="Navigation menu">
        <div className="flex items-center justify-between px-6 h-20">
          <span className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10 border border-white/20 text-sm font-bold text-white">A</span>
            <span className="text-xl font-extrabold text-white tracking-tight" style={{ fontFamily: 'var(--font-landing)' }}>AlphaSeekers</span>
          </span>
          <button onClick={closeMobile} className="flex h-11 w-11 items-center justify-center rounded-xl text-white/60 hover:text-white hover:bg-white/10 transition-colors" aria-label="Close menu">
        ...
        <div className="px-8 pb-12 space-y-3">
          ...
          {!signedIn && <Link href={`/${locale}/login`} onClick={closeMobile} className="block w-full py-4 text-center rounded-xl border-2 border-white/20 text-white text-base font-semibold hover:bg-white/10 transition-colors">{loginLabel}</Link>}
