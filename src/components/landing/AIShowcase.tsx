...
const chatMessages = [
  ...
  {
    role: 'ai' as const,
    text: 'فتوسنتز فرآیندی است که گیاهان با استفاده از نور خورشید، آب و دی‌اکسید کربن، غذا (گلوکز) و اکسیژن تولید می‌کنند. این فرآیند در کلروپلاست‌های برگ انجام می‌شود.',
    source: 'Chapter 3: Plant Biology',
  },
];
...
function TypingText({ text, shouldAnimate }: { text: string; shouldAnimate: boolean }) {
              ...
              <div className="flex items-center gap-3 pb-4 border-b border-white/[0.06]">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-land-green-400" />
              ...
              <div className="py-6 space-y-4" dir="rtl">
                ...
                <div className="flex justify-start">
                  <div className="bg-land-green-600/20 rounded-2xl rounded-tr-md px-4 py-3 max-w-[85%]">
                    <p className="text-sm text-white">{chatMessages[0].text}</p>
                  </div>
                </div>
                  ...
                  <div className="max-w-[85%]">
                    <div className="bg-white/[0.05] rounded-2xl rounded-tl-md px-4 py-3">
                      <p className="text-sm text-white/80 leading-relaxed">
                        <TypingText text={chatMessages[1].text} shouldAnimate={hasTriggered} />
                      </p>
                    </div>
                    {/* Source citation */}
                    <p className="text-xs text-white/30 mt-2 text-left" dir="ltr">
                      Sources: {chatMessages[1].source}
                    </p>
                    {/* Feedback buttons */}
          ...
          <div className="grid grid-cols-2 gap-4 mt-12 lg:mt-0">
            {features.map((f, i) => (
              <ScrollReveal key={f.title} delay={i * 0.1}>
                <div className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-5 hover:bg-white/[0.08] hover:border-white/[0.12] transition-all duration-300">
                  <svg className="w-5 h-5 text-land-green-400 mb-3" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
