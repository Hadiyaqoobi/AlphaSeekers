  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-10 lg:px-8">
      ...
      <header className="mb-10 flex items-start gap-4">
        <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl bg-sky-500/10 text-sky-400 ring-1 ring-sky-500/20">
            ...
            >
              ...
              <div className="flex flex-1 flex-col px-6 pb-6 pt-4">
                ...
                {user && (user.role === "ADMIN" || (user.approved && registeredIds.has(item.id))) ? (
                  <a
                    className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-neon-400 transition hover:text-neon-300"
                    href={item.meetLink}
                    rel="noreferrer"
                    target="_blank"
                  >
                  ...
                  <p className="mt-4 text-sm text-ink-faint">{t("registerPrompt")}</p>
                )}
                ...
                <div className="mt-5 flex items-center justify-between gap-2 border-t border-white/5 pt-4">
                  {user ? (
                    registeredIds.has(item.id) ? (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-neon-500/10 px-3 py-1 text-xs font-semibold text-neon-300">
