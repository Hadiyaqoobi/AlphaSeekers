...
const PROGRESS_COLORS = [
  { bar: "from-emerald-400 to-teal-500", badge: "bg-emerald-50 text-emerald-700" },
...
export default async function ProfilePage({ params }: ProfilePageProps) {
    ...
    return (
      <section className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="h-24 bg-gradient-to-r from-teal-600 via-emerald-600 to-cyan-600" />
          <div className="relative px-6 pb-6">
            <div className="-mt-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex items-end gap-4">
                <div className="flex h-20 w-20 items-center justify-center rounded-2xl border-4 border-white bg-gradient-to-br from-teal-500 to-emerald-600 text-2xl font-bold text-white shadow-lg">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="mb-3 text-sm font-bold uppercase tracking-wider text-slate-400">Integrations</h2>
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {/* Gradient banner */}
        <div className="relative h-28 bg-gradient-to-r from-teal-600 via-emerald-600 to-cyan-600 sm:h-32">
        ...
        <div className="relative px-5 pb-5 sm:px-6 sm:pb-6">
          <div className="-mt-12 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            {/* Avatar + Name */}
            <div className="flex items-end gap-4">
              <div className="flex h-24 w-24 items-center justify-center rounded-2xl border-4 border-white bg-gradient-to-br from-teal-500 to-emerald-600 text-3xl font-bold text-white shadow-xl">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
          <div className="absolute -right-3 -top-3 h-16 w-16 rounded-full bg-emerald-50" />
          <div className="relative">
            ...
            <p className="mt-3 text-2xl font-extrabold text-slate-900">{profile.stats.activeClasses}</p>
            <p className="mt-0.5 text-sm font-medium text-slate-500">{t("stats.activeClasses")}</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
          ...
          <div className="relative">
            ...
            <p className="mt-3 text-2xl font-extrabold text-slate-900">{profile.stats.sessionsThisWeek}</p>
            <p className="mt-0.5 text-sm font-medium text-slate-500">{t("stats.thisWeek")}</p>
            <p className="mt-0.5 text-xs text-slate-400">{t("stats.upcomingSessions")}</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
          ...
          <div className="relative">
            ...
            <p className="mt-3 text-2xl font-extrabold text-slate-900">{profile.stats.totalMaterials}</p>
            <p className="mt-0.5 text-sm font-medium text-slate-500">{t("stats.materials")}</p>
            <p className="mt-0.5 text-xs text-slate-400">{t("stats.availableAcrossClasses")}</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
          ...
          <div className="relative">
            ...
            <p className="mt-3 text-2xl font-extrabold text-slate-900">{profile.stats.notifications}</p>
            <p className="mt-0.5 text-sm font-medium text-slate-500">{t("stats.alerts")}</p>
          </div>
        </div>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-5 py-4 sm:px-6">
          <h2 className="text-lg font-bold text-slate-900">{t("progress.title")}</h2>
        </div>
            ...
            return (
              <article className="px-5 py-4 sm:px-6" key={item.id}>
                ...
                {item.nextSessionStart ? (
                  <p className="mt-1.5 inline-flex items-center gap-1 text-xs font-medium text-slate-700">
                  ...
                  <p className="mt-1.5 text-xs text-slate-400">{t("progress.nextPending")}</p>
                )}
              </article>
            );
          })}
              ...
              <div className="relative space-y-0">
                ...
                {profile.upcoming.map((item, idx) => (
                  <article className="relative pl-7 pb-6 last:pb-0" key={item.id}>
                    {/* Timeline dot */}
                    <div className={`absolute left-0 top-1.5 h-[15px] w-[15px] rounded-full border-[3px] border-white shadow-sm ${idx === 0 ? "bg-teal-500" : "bg-slate-300"}`} />
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="divide-y divide-slate-50">
            ...
            {profile.notifications.map((item) => (
              <article className="flex gap-3 px-5 py-3.5 transition-colors hover:bg-slate-50/50 sm:px-6" key={item.id}>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-5 py-4 sm:px-6">
          <h2 className="text-lg font-bold text-slate-900">{t("opportunities.title")}</h2>
        </div>
        <div className="p-5 sm:p-6">
          {profile.opportunities.length === 0 ? (
            <p className="py-6 text-center text-sm text-slate-500">{t("opportunities.empty")}</p>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2">
              {profile.opportunities.map((item) => (
                <article
                  className="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-4 transition-all hover:shadow-md hover:-translate-y-0.5"
                  key={item.id}
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
